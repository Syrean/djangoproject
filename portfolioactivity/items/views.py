...
def retrieve_items(request):
    ...
    matched_items = [item for item in items if search_term.lower() in item['name'].lower()]
    return JsonResponse({'items': matched_items if search_term else items}, status=200)
